...
estaciones = [
    ...
    {"nombre": "IESTorrevigia - Torrevieja", "latitud": "37.998659", "longitud": "-0.695838", "url": "http://www.datos.eltiempoentorrevieja.es/IESTorrevigia/iestorrevigia"},
    ...
    {"nombre": "Salinas Isla - Torrevieja - Torrevieja", "latitud": "37.991396", "longitud": "-0.720184", "url": "http://www.datos.eltiempoentorrevieja.es/Salinas_Isla/salinas2"},
...
for estacion in estaciones:
    try:
        ...
        velocidad_viento = root.find("ns:actuales/ns:velocidadviento", NAMESPACE)

        if fecha is not None and hora is not None and velocidad_viento is not None:
            if es_dato_actualizado(fecha.text, hora.text):  # Validamos la hora
                datos_estaciones.append({
                    ...
                    "fecha": fecha.text,
                    "hora": hora.text,
                    "velocidad_viento": velocidad_viento.text
                })
            else:
...
with open("datos_meteorologicos.json", "w", encoding="utf-8") as f:
    json.dump(datos_finales, f, indent=4, ensure_ascii=False)

print(f"✅ Datos meteorológicos guardados en 'datos_meteorologicos.json' con fecha: {fecha_actual_str}")